...
libname mine '/scratch/kaist/junyoup'; *my temporary storage on wrds.;

/*upload ccmxpf_linktable and secuity table. security table under comp\sasdata\naa;*/
...
keep GVKEY conm datadate fyear fyr che at dltt sale oibdp xint txt dvc csho dlc xrd ceq capx
     aqc wcap prcc_f sic sich cshpri ib txdi itci sstk prstkc dltis dltr; *add other variables. NI/AT=ROA, NI/CEQ = ROE, NI/ICAPT = ROI;
run;
...
/*upload ccmxpf_linktable to mine library*/
proc sql;
  ...
  from comp1 as a, mine.ccmxpf_linktable as b
...
proc expand data = crsp2 out=crsp3 method=none;
    by permno;
    id date;
    convert RET = RET_3  / transformin=(+1) transformout=(MOVPROD  3 -1 trimleft  3);
    ...
    convert RET = Tot_Vol / transformout = (movstd 24 trimleft 24); *vary months for total volatility;
quit;
proc printto; run;
...
/*all variables including returns*/
data mine.compu_crsp; set compu;run;

data mine.compu;
set compu;
keep GVKEY permno permco conm datadate fyear fyr che at dltt sale oibdp xint txt dvc csho dlc xrd ceq capx
     aqc wcap prcc_f sic sich cshpri ib txdi itci sstk prstkc dltis dltr;
run;
